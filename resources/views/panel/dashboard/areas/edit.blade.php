    ...
    <div class="content-wrapper">
        ...
        <form action="{{ route('areas.update.area') }}" method="POST" >
            ...
            <div class="form-inline" >
                ...
                <input type="text" id="title" class="form-control rounded-input" name="title" placeholder="اسم المنطقة" value="{{ old('title') ? old('title') : $area->title }}" style="margin-left: 10px;">
                
                <select name="city_id" class="form-control rounded-input" style="margin-left: 10px;padding-left:5.0rem">
                    <option value="">اختر المدينة</option>
                    @foreach ($cities as $city)
                    <option value='{{$city->id}}' {{ (old('city') == $city->title || $area->city->title == $city->title) ? 'selected' : '' }} > {{$city->title}} </option>
                    @endforeach
                </select>
