        ...
        <div class="content-body">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px; width: 50%; margin: inherit;">
                ...
                <form id="myForm" action="{{Route('monitors.update')}}" method="post" style="display: flex; flex-direction: column; gap: 10px;">
                    ...
                    @enderror
                    <input type="text" oninput="sendName()" id="nameInput" name='name' value="{{old('name')?old('name'):($name?$name:$monitor->user->name)}}" style="padding: 10px; width: 100%; border-radius: 5px; border: 1px solid #ccc;">
                    @if (Auth::User()->type == "admin")
                            <label for="mobileInput">تعديل رقم الهاتف </label>
                            @error('mobile')
                            <p style="color: red" > * {{$message}}</p>
                            @enderror
                            <input type="text" oninput="sendMobile()" id="mobileInput" name='mobile' value="{{old('mobile')?old("mobile"):($mobile?$mobile:$monitor->user->mobile)}}" style="padding: 10px; width: 100%; border-radius: 5px; border: 1px solid #ccc;">
                            
                            <label for="emailInput">تعديل البريد الالكتروني </label>
                            @error('email')
                            <p style="color: red" > * {{$message}}</p>
                            @enderror
                            <input type="text" oninput="sendEmail()" id="emailInput" name='email' value="{{old("email")?old("email"):($email?$email:$monitor->user->email)}}" style="padding: 10px; width: 100%; border-radius: 5px; border: 1px solid #ccc;">
                    @endif
                </form>
